...
bool D3DClass::Initialize(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	//팩토리 객체를 사용하여 첫번쨰 그래픽 카드 인터페이스 어텝터를 생성
	...
	//모니테어 대한 DXGI_FORMAT_R8G8B88A8_UNORM 표시 형식에 맞는 모드 수를 가져오기
	...
	//모든 디스플레이 모드에 대해 화면 너비/높이에 맞는 디스플레이 모드를 찾기
	...
	//설정된 깊어버퍼 구조체를 사용하여 깊어 버퍼 텍스쳐를 생성
	...
	//3D 랜더링을위한 투영 행렬을 만들기
	m_projectionMatrix = XMMatrixPerspectiveFovLH(fieldOfView, screenAspect, screenNear, screenDepth);

	//세계 행렬을 항등 행렬로 초기화
	m_worldMatrix = XMMatrixIdentity();

	//2D 렌더링을위한 직교 투영 행렬을 만듬
	m_orthoMatrix = XMMatrixOrthographicLH((float)screenWidth, (float)screenHeight, screenNear, screenDepth);

	//이제 2D 렌더링을 위한 Z 버퍼를 끄는 두 번째 깊이 스텐실 상태를 만듬. 유일한 차이점은
	//DepthEnable을 false로 설정하면 다른 모든 매개 변수는 다른 깊이 스텐실 상태와 동일
...
{
	//종료 전 윈도우 모드로 설정하지 않으면 스왑 체인을 해제 할 때 예외가 발생한다고 한다.
	if (m_swapChain)
	{
...
{
	float blendFactor[4] = { 0.0f, 0.0f, 0.0f };
