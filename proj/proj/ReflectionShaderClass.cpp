...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, (WCHAR*)L"../proj/Reflection.vs", (WCHAR*)L"../proj/Reflection.ps");
}
...
bool ReflectionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "Reflection", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	result = D3DCompileFromFile(psFilename, NULL, NULL, "Reflection", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
		// 셰이더 컴파일 실패시 오류메시지를 출력합니다.
		if (errorMessage)
	...
	pixelBufferDesc.ByteWidth = sizeof(BufferType);
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* reflectionTexture,XMMATRIX reflectionMatrix)
{
	...
	BufferType* dataPtr2 = (BufferType*)mappedResource.pData;
