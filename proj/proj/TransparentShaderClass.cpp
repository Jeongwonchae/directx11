...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, (WCHAR*)L"../proj/transparent.vs", (WCHAR*)L"../proj/transparent.ps");
}
...
bool TransparentShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "MyVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(result))
	{
	...
	pixelBufferDesc.ByteWidth = sizeof(TransparentBufferType);
	...
	result = device->CreateBuffer(&pixelBufferDesc, NULL, &m_transparentBuffer);
	if (FAILED(result))
	{
		return false;
	}
